...
import {Message, MessageMedia, Contact, Location, Buttons, List } from 'mywajs';
import fetch from "node-fetch";
import { writeExifImg } from "./sticker.js";
 ...
 * @param {Object} quoted
 */
async sendDocs(jid, link, namefile= '', caption = '') {
...
* @param {*} quoted
*/
async sendSticker(jid, image, packname = 'Stiker dibuat oleh:', author = 'BerkahEsport.ID', quoted) {
  let m;
  try {
    m = await conn.sendMessage(jid, image, {
      asSticker: true,
      quoted: quoted ? quoted : '', 
      packName: packname, 
      packPublish: author
    ...
    if (!m) m = await conn.sendMessage(jid, image, {
      asSticker: true,
      quoted: quoted ? quoted : '',
      packName: 'Stiker dibuat oleh:',
      packPublish: 'BerkahEsport.ID'
    })
}
}
/**
 ...
 * @param {String} quoted
 * @returns {Promise<import("mywajs").Message>} Message that was just sent
 */
async sendFiles(jid, link = '', namefile = '', caption = '', quoted = "", doc = false) {
 let m;
  try { 
    let type = await conn.getFile(link, true)
    let {filename, ext } = type
 ...
 */
async sendFile(chatId, content, filename = "", captions = "", options = {}, ptt = false) {
  let internalOptions = {
